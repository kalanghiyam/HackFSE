...
public interface EventEmployeeInfoRepository extends JpaRepository<EventEmployeeInfo, EventEmployeeIdentity>{
	...
	@Query("select info.eventEmployeeIdentity.eventId,info.eventStatus,count(info.eventStatus) from EventEmployeeInfo info inner join  info.eventSummaryDetails details"
			+ " group by info.eventEmployeeIdentity.eventId,info.eventStatus order by info.eventDate desc")
	List<Object[]> getEventDashboardReport();
	
	@Query("select info.eventEmployeeIdentity.eventId,info.eventStatus,count(info.eventStatus) from EventEmployeeInfo info inner join  info.eventSummaryDetails details"
			+ " where details.pocId=:pocId group by info.eventEmployeeIdentity.eventId,info.eventStatus  order by info.eventDate desc")
	List<Object[]> getEventDashboardReportForPoc(@Param("pocId") String pocId);
